...
function ComingSoon({ title }: { title: string }) { return <div className="coming-soon"><Sparkles size={28}/><span className="eyebrow">STUDENT WORKSPACE</span><h1>{title}</h1><p>This module is next in the Hayagriva Vidya Kendram build. Your existing data model is already designed to connect here.</p></div>; }
...
function AdminWorkspace({ user, logout }: { user: User; logout: () => void }) {
  ...
  return <main className="erp-shell"><header className="erp-top"><div className="brand"><div className="brand-mark">H</div>Hayagriva Vidya Kendram</div><span>Administrator ERP</span><button className="logout" onClick={logout}><LogOut size={16}/> Sign out</button></header><div className="erp-page">
    ...
    <div className="module-grid admin-grid" style={{ marginTop: '24px' }}>
      ...
      <article className="card"><span className="eyebrow">ONBOARDING FLOW</span><h2>Secure first access</h2><ol className="onboarding-steps"><li>Admin creates the account and temporary password.</li><li>Hayagriva Vidya Kendram emails the login link and credentials to the real address.</li><li>The recipient signs in using the temporary password.</li><li>Hayagriva Vidya Kendram immediately requires a new password before access.</li></ol><p className="access-note">SMTP credentials are kept in <code>.env.local</code>, never in the admin interface or SQLite database.</p></article>
    </div>
  </div></main>;
}
...
function ParentWorkspace({ user, logout }: { user: User; logout: () => void }) {
  ...
  return <main className="erp-shell"><header className="erp-top"><div className="brand"><div className="brand-mark">H</div>Hayagriva Vidya Kendram</div><span>Parent Portal</span><button className="logout" onClick={logout}><LogOut size={16}/> Sign out</button></header><div className="erp-page">
  ...
  return (
    <main className="erp-shell">
      <header className="erp-top">
        <div className="brand"><div className="brand-mark">H</div>Hayagriva Vidya Kendram</div>
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    fetch('/api/student/overview').then(r => {
      if (!r.ok) {
  ...
  if (!data) return <main className="loading"><div className="brand"><div className="brand-mark">H</div>Hayagriva Vidya Kendram</div><p>Loading your intelligence layer…</p></main>;
